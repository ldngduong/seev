...
export type ResearchMode = 'quick' | 'custom' | 'external'
...
export function useResearchCvPage() {
  ...
  const handleCreated = (session: { id: string }) => { toast.success('Đã bắt đầu research CV.'); void queryClient.invalidateQueries({ queryKey: ['billing', 'account'] }); void queryClient.invalidateQueries({ queryKey: ['dashboard', 'me'] }); void navigate(`/research-history/${session.id}`) }
