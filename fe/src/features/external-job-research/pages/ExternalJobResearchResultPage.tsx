...
export function ExternalJobResearchResultPage() {
  ...
  return <main className="flex w-full flex-col gap-5">
    <DashboardPageHeader title={<div className="min-w-0"><h1 className="break-words text-xl font-semibold tracking-tight text-zinc-800 sm:text-2xl">Đánh giá theo {research.source_kind === 'link' ? 'liên kết tuyển dụng' : 'JD'}</h1><p className="mt-1 break-all text-sm text-muted-foreground">Đối chiếu với CV: {research.cv?.name ?? 'CV đã chọn'}</p></div>} actions={<><Link to="/research-history" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }))}>Lịch sử research</Link></>} />
