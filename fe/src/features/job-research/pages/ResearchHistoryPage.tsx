...
export function ResearchHistoryPage() {
  ...
  return <main className="flex w-full flex-col gap-5">
    ...
    <section className="grid gap-3">
      <Tabs value={history.type} onValueChange={(value) => history.setType(value as 'quick' | 'custom' | 'job_fit')}>
        <TabsList className="grid w-full grid-cols-3 rounded-xl p-1 group-data-horizontal/tabs:h-10" variant="default">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <label className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={history.search} onChange={(event) => history.setSearch(event.target.value)} placeholder={history.type === 'job_fit' ? 'Tìm theo CV, việc làm hoặc công ty' : 'Tìm theo CV hoặc vị trí mục tiêu'} className="pl-9" /></label>
      ...
      {!history.isLoading && !history.sessions.length && !history.jobFits.length ? <EmptyResearchHistory /> : null}
