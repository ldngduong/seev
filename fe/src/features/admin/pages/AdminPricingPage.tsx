...
function PriceRow({ service, update, pending }: { service: ServiceProduct; update: (price: number) => void; pending: boolean }) {
  ...
  return <article className="grid gap-4 rounded-2xl border border-border/60 bg-card p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
    <div className="min-w-0"><h2 className="font-semibold text-zinc-800">{service.name}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{service.description}</p></div>
    <div className="grid items-end gap-3 sm:grid-cols-[180px_auto]">
      <label><span className="mb-1.5 block text-xs text-muted-foreground">Giá mỗi lượt</span><div className="relative"><Input type="number" min={0} value={price} onChange={(event) => setPrice(event.target.value)} className="pr-14" /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">credit</span></div></label>
      <Button disabled={pending || price === '' || Number(price) < 0} onClick={() => update(Number(price))}>Lưu thay đổi</Button>
    </div>
  </article>
}
