...
export function AdminUserDetailPage() {
  ...
  return <AdminPage title={detail.user.fullName} description={detail.user.email}>
    <section className="grid gap-3 sm:grid-cols-3">
      ...
      <MetricCard label="Phiên nghiên cứu" value={detail.sessions.length} />
      <MetricCard label="CV đã lưu" value={detail.cvs.length} />
    </section>
    ...
    <section className="grid gap-3 lg:grid-cols-2">
      <HistoryPanel icon={WalletCards} title="Lịch sử credit" empty="Chưa có giao dịch credit.">
        {detail.transactions.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 border-b border-border/60 py-3 last:border-0">
